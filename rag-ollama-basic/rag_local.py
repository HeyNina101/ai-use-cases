...
from langchain_community.document_loaders import PyPDFLoader # Or UnstructuredPDFLoader / PyPDFLoader
from langchain_text_splitters import RecursiveCharacterTextSplitter # For splitting text into chunks
from langchain_ollama import OllamaEmbeddings # Ollama embeddings for text representation
from langchain_community.vectorstores import Chroma # database for storing embeddings
...
def create_rag_chain(vector_store, llm_model_name=MODEL, context_window=8192):
...
if __name__ == "__main__":
    ...
    # 5. Create RAG Chain
    rag_chain = create_rag_chain(vector_store, llm_model_name=MODEL) # if has RAM < 8GB Use the chosen Qwen 3 model
